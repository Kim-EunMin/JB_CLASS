# 클래스 생성과 객체생성 ( 초기화 함수 활용 )

class 빵틀:
    모양 = str()
    반죽 = str()
    앙꼬 = str()
    단가 = int()

    #__init__ 을 안쓴다면?   -> 써야하는게 많아진다.
    # 예:   붕어빵 = 빵틀()  객체생성완료
    #      붕어빵.모양 = '붕어빵'
    #      붕어빵.반죽 = '밀가루'        -> 이런식으로 값을 전부 지정해줘야한다.

    #__init__을 쓴다면 -> 붕어빵 = 빵틀(붕어빵,밀가루,팥,200)  -> class 호출할때 안에 넣어주면 된다.(1줄로끝남)

    def __init__(self, 모양, 반죽, 앙꼬, 단가):
        self.모양 = 모양
        self.반죽 = 반죽
        self.앙꼬 = 앙꼬
        self.단가 = 단가

    def 굽기(self, 주문갯수):
        굽는횟수 = (주문갯수-1) / (10+1)
        완성시간 = int(굽는횟수) * 5
        return 완성시간

    def 가격(self, 주문갯수):
        금액 = 주문갯수 * self.단가
        return 금액

    def 주문(self, 주문갯수, 지불금액):
        대기시간 = self.굽기(주문갯수)  #클래스 안에 정의된 함수니까 -> self.함수 하면 된다.
        주문금액 = self.가격(주문갯수)
        거스름돈 = 지불금액 - 주문금액
        return 대기시간, 거스름돈


# 다꼬야끼 주문
print("타코야끼 주문 (1개 500원)")
다코야끼 = 빵틀('다꼬야끼', '밀가루','낙지',500)

order = 20  #주문갯수
payment = 10000  #지불금액

wait_time, change = 다코야끼.주문(order, payment)  #주문함수에 return 값은 2개이다.

shape = 다코야끼.모양

print('{}빵 {}개를 주문하였고, {}원을 지불하였습니다.'.format(shape, order, payment))


if change == 0:
    message = '손님, {wt} 분만 기다려주세요'.format(wt = wait_time)
elif change>0:
    message = f'잔돈은 {ch}원 입니다 {wt}분만 기다려주세요'.format(ch = change, wt = wait_time)
elif change<0:
    message = '손님, 금액이 {ch} 원 부족합니다.'.format(ch=-change)
else:
    message = 'Error'




print(message)
